...
func InitDatabase() {
	...
	if dal.ExistColumnInTable("applications", "oauth_required") == false {
		...
		if err != nil {
			utils.DebugPrintln("InitDatabase applications oauth", err)
		}
	}
	...
	if dal.ExistColumnInTable("applications", "csp") == false {
		// v0.9.11 CSP
		err = dal.ExecSQL(`alter table applications add column csp_enabled boolean default false, add column csp varchar(1024) default E'default-src \'self\''`)
		if err != nil {
			utils.DebugPrintln("InitDatabase applications", err)
		}
	}
